...
public class BashekimGUI extends JFrame {
	...
	public BashekimGUI(Bashekim bashekim) throws SQLException {
		...
		w_tab.addTab("Tüm Randevular", null, w_appointment, null);
		...
		table_doktor = new JTable(doktorModel);
		w_scrollDoctor.setViewportView(table_doktor);
		
		JLabel lblNewLabel_1 = new JLabel("Doktor Listesi");
		lblNewLabel_1.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblNewLabel_1.setBounds(10, 0, 126, 25);
		w_appointment.add(lblNewLabel_1);
		
		JLabel lblPoliklinikAdi_1 = new JLabel("Poliklinik Adi");
		lblPoliklinikAdi_1.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblPoliklinikAdi_1.setBounds(270, 0, 126, 25);
		w_appointment.add(lblPoliklinikAdi_1);
				...
				if (select_Clinic.getSelectedIndex() != 0) {
					...
					try {
						for (int i = 0; i < clinic.getClinicDoctorList(item.getKey()).size(); i++) {
							doktorData[0] = clinic.getClinicDoctorList(item.getKey()).get(i).getId();
							doktorData[1] = clinic.getClinicDoctorList(item.getKey()).get(i).getName();
							doktorModel.addRow(doktorData);
		...
		btn_selDoc.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (row >= 0) {
					...
					selectDoctorID = id;
					selectDoctorName = table_doktor.getModel().getValueAt(row, 1).toString();
				} else {
